...
function MintWidget() {
  const { ethers, contract, account, setAlert, setAlertType, _connectWallet } = useContext(Web3Context)
  ...
  if (loading && !mintSucessful) {
    mintAction = (
      <svg className="animate-spin mx-auto h-6 w-6 text-white" viewBox="0 0 24 24">
    ...
    mintAction = (
      <>
        <select className="rounded-md w-14 text-center" name="mintAmount" id="mintAmount" onChange={handleMintAmount} defaultValue="1">
    ...
    mintAction = <span className="text-theme">You have minted successfully!</span>
  }
    ...
    <div className="max-w-xl mx-auto">
      <div className="bg-secondary-theme rounded-md p-8 flex justify-center items-center text-black">
        <div className="flex flex-col space-y-8">
          <h2 className="text-3xl font-bold text-white text-center">Mint Details</h2>
          ...
          {account && <div className="flex justify-center space-x-4">{mintAction}</div>}
        </div>
      </div>
    </div>
  )
