...
function getMintPrice(mintAmount) {
  return ethers.utils.parseEther(CollectionConfig.price.toString()).mul(mintAmount)
}
...
describe(CollectionConfig.contractName, function () {
  ...
  it("Contract deployment", async function () {
    ...
    contract = await Contract.deploy(...ContractArguments)
  ...
  it("Check initial data", async function () {
    ...
    await expect(contract.tokenURI(1)).to.be.revertedWith("ERC721AMetadata: URI query for nonexistent token")
  })
  ...
  it("Before any sale", async function () {
    ...
    await expect(contract.connect(whitelistedUser).mint(1, { value: getMintPrice(1) })).to.be.revertedWith("The contract is paused!")
    await expect(contract.connect(whitelistedUser).whiteListMint([], 1, { value: getMintPrice(1) })).to.be.revertedWith("Whitelist sale is not enabled!")
    await expect(contract.connect(holder).mint(1, { value: getMintPrice(1) })).to.be.revertedWith("The contract is paused!")
    await expect(contract.connect(holder).whiteListMint([], 1, { value: getMintPrice(1) })).to.be.revertedWith("Whitelist sale is not enabled!")
    await expect(contract.connect(owner).mint(1, { value: getMintPrice(1) })).to.be.revertedWith("The contract is paused!")
    await expect(contract.connect(owner).whiteListMint([], 1, { value: getMintPrice(1) })).to.be.revertedWith("Whitelist sale is not enabled!")
    ...
    await expect(contract.mintForAddress(await (await contract.maxMintAmountPerTx()).add(1), await holder.getAddress())).to.be.revertedWith("Invalid mint amount!")
  ...
  it("Whitelist Sale", async () => {
    ...
    const merkleProof = tree.getHexProof(keccak256(await whitelistedUser.getAddress()))

    // enable whitelist mint
    await contract.setWhitelistMintEnabled(true)

    // mint without paying ether
    await expect(contract.connect(whitelistedUser).whiteListMint(merkleProof, 2)).to.be.rejectedWith(Error, "insufficient funds for intrinsic transaction cost")
    ...
    await expect(
      contract.connect(holder).whiteListMint(tree.getHexProof(keccak256(await holder.getAddress())), 2, {
        value: getMintPrice(2),
      })
    ).to.be.revertedWith("Invalid proof!")

    // Sending no proof at all
    await expect(contract.connect(holder).whiteListMint([], 1, { value: getMintPrice(1) })).to.be.revertedWith("Invalid proof!")
  ...
  it("Not allow normal mint while whitelist mint is open", async () => {
    // enable whitelist mint
    await contract.setWhitelistMintEnabled(true)
    expect(contract.connect(externalUser).mint(3)).to.be.revertedWith("The contract is paused!")
  })
  ...
  it("Public sale", async function () {
    ...
    await expect(contract.connect(holder).mint(1, { value: getMintPrice(1).sub(1) })).to.be.rejectedWith(Error, "insufficient funds for intrinsic transaction cost")
    // Sending an invalid mint amount
    await expect(
      contract.connect(whitelistedUser).mint(await (await contract.maxMintAmountPerTx()).add(1), {
        value: getMintPrice(await (await contract.maxMintAmountPerTx()).add(1).toNumber()),
      })
    ).to.be.revertedWith("Invalid mint amount!")
    // Sending a whitelist mint transaction
    await expect(contract.connect(whitelistedUser).whiteListMint([], 1, { value: getMintPrice(1) })).to.be.revertedWith("Address already claimed!")
  ...
  it("Owner only functions", async function () {
    ...
    await expect(contract.connect(externalUser).setCost(ethers.utils.parseEther("0.0000001"))).to.be.revertedWith("Ownable: caller is not the owner")
  ...
  it("Token URI generation", async function () {
    ...
    await expect(contract.tokenURI(0)).to.be.revertedWith("ERC721AMetadata: URI query for nonexistent token")
